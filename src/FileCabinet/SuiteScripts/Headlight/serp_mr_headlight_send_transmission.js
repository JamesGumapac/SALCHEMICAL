...
define([
  "N/runtime",
  "N/search",
  "./Lib/serp_headlight_send_orders.js",
  "./Lib/serp_headlight_api_service.js",
]
/**
 ...
 */, (runtime, search, add_order_util, APIService) => {
  ...
  const getInputData = (inputContext) => {
    try {
      ...
      const headlightIntegrationSettings =
        add_order_util.getHeadlightIntegSettings(headlightIntegrationSettingId);
      let orderObj = add_order_util.getPendingFulfillmentItem({
        searchId: params.soSearchId,
        deleteOrders: headlightIntegrationSettings.deleteOrders,
      });

      log.debug("orderObj length", orderObj.orders.length);
      ...
      if (response.successfully_created_orders.length > 0) {
        response.successfully_created_orders.forEach((data) =>
          ordersToBeUpdated.push({ sucessfulOrders: data })
        );
      }
